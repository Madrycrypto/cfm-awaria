...
const WEBHOOK_REPORT = 'https://ocn7yfudom36.jp.feishu.cn/base/workflow/webhook/event/ZrobapVv3wZ91VhphJ0joUwXpiQ'; // RaportDzienny
const WEBHOOK_QUALITY = 'https://ocn7yfudom36.jp.feishu.cn/base/workflow/webhook/event/OmoIaHIzewO2tYhvJLvj0TPQpBh'; // PrzyczynyJakosc
const WEBHOOK_AWARIA = 'https://ocn7yfudom36.jp.feishu.cn/base/workflow/webhook/event/HL1taj5vdwhM8xhle7QjY9hgpHb'; // Awarie
const WEBHOOK_REWORK = 'https://ocn7yfudom36.jp.feishu.cn/base/workflow/webhook/event/T5MQaNgSAweEFkhNmZMjckZhpig'; // ReworkProcessing
// ─────────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    if (path === '/api/awaria' && req.method === 'POST') {
      ...
      await forwardToFeishu(WEBHOOK_AWARIA, {
        event: body.event || '', station: body.station || '', operator: body.operator || '', type: body.type || '',
        start_timestamp: body.start_timestamp || '', koniec_timestamp: body.koniec_timestamp || '',
        czas_min: body.czas_min || '', status: body.status || '',
      });
      res.end(JSON.stringify({ status: 'ok' }));
      return;
    }
